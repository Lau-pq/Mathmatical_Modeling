...
plt.rc('font', size=12)
x1, x2 = sp.var('x1, x2') # 定义符号变量
y = (339 - 0.01 * x1 - 0.003 * x2) * x1 + (339 - 0.04 * x1 - 0.01 * x2) * x2 - (400000 + 195 * x1 + 225 * x2)
y = sp.simplify(y) # 化简
...
contr = plt.contour(X, Y, Z, 10) # 10 条等高线
plt.clabel(contr) # 等高线标注
plt.ylabel('$x_2$', rotation=0)
plt.xlabel('$x_1$')
a = sp.var('a', pos=True) # 定义灵敏度分析的符号参数
y = (339 - a * x1 - 0.003 * x2) * x1 + (399 - 0.004 * x1 - 0.01 * x2) * x2 - (400000 + 195 * x1 +225 * x2)
y = sp.simplify(y) # 化简
dy1 = y.diff(x1) # 求关于 x1 的偏导
dy2 = y.diff(x2) # 求关于 x2 的偏导
s = sp.solve([dy1, dy2], [x1, x2])
sx1 = s[x1]; sx2 = s[x2] # 提取解分量
s1 = sp.lambdify('a', sx1, 'numpy') # 符号函数转化为匿名函数
...
sx2a = dx20 * 0.01 / 7043
Y = y.subs({x1:s[x1], x2:s[x2]}) # 求关于 a 的目标函数
